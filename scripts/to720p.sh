...
# example usage:
#   to720p.sh "input.mp4" "output.mp4" "00:00:00" "01:03:24"
...
#ffmpeg -i "$1" -c:v libx264 -c:a aac       -b:a 192k -b:v 1.5M  -s 1280x720 -ss "$3" -to "$4" "$2" 
#ffmpeg -i "$1" -c:v libx264 -c:a ac3       -b:a 384k -b:v 1.5M  -s 1280x720 -ss "$3" -to "$4" "$2" 

#ffmpeg -i "$1" -c:v libx264 -c:a copy                -b:v 2000k -s 1280x720 -ss "$3" -to "$4" "$2" 
