#!/bin/bash
# Original Author: klaxalk (klaxalk@gmail.com, github.com/klaxalk)
# For Author: tzcrawford (contact@tzcrawford.com)
# Dependencies:
...
#ROFI_BIN="$(whereis -b dmenu | awk '{print $2}')"
ROFI_BIN="$(whereis -b ~/scripts/dmenu.sh | awk '{print $2}')"
...
if [ -z "$ROFI_BIN" ]; then
  echo missing rofi/dmenu, please install dependencies
  exit 1
fi
...
SAVE LAYOUT
DELETE LAYOUT" | dmenu -y -p "Select action")
#DELETE LAYOUT" | rofi -i -dmenu -no-custom -p "Select action")
  ...
  LAYOUT_NAME=$(echo "$LAYOUT_NAMES" | dmenu -y -p "Select layout (you may type new name when creating)" | sed 's/\s/_/g') # ask for selection
...
SPECIFIC (CHOOSE)
MATCH ANY" | dmenu -y -p "How to identify windows? (xprop style)")
  ...
  if [[ "$CRITERION" = "default" ]]; then
    ...
    while true; do
      ...
      if [ ! -z "$LINE_NUM" ]; then
        ...
        SELECTED_OPTION=$(cat -n $LAYOUT_FILE | sed -n "${LINE_NUM},$(expr $LINE_NUM + 2)p" | awk '{$2="";print $0}' | -y -p "Choose the matching method for${NAME%,}" | awk '{print $1}')
